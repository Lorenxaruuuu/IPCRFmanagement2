...
<body class="bg-slate-50 font-sans text-slate-900 h-screen overflow-hidden flex" 
      x-data='{ 
          activeTab: localStorage.getItem("userActiveTab") || "home", 
      ...
      x-init="$watch('activeTab', value => { localStorage.setItem('userActiveTab', value); $nextTick(() => lucide.createIcons()); })">
    ...
    <aside class="fixed left-0 top-0 h-full w-64 sidebar-gradient text-white hidden md:flex flex-col shadow-xl z-20">
        ...
        <nav class="flex-1 py-6 px-0 space-y-1">
            <button @click="activeTab = 'home'" 
                    class="w-full flex items-center gap-3 px-6 py-3.5 text-base transition-all text-left cursor-pointer border-l-4"
            ...
            <button @click="activeTab = 'forms'" 
                    class="w-full flex items-center gap-3 px-6 py-3.5 text-base transition-all text-left cursor-pointer border-l-4"
                    :class="activeTab === 'forms' ? 'bg-white/10 border-blue-500 text-white font-semibold shadow-sm' : 'border-transparent text-gray-300 hover:bg-white/5 hover:text-white'">
            ...
            <button @click="activeTab = 'notifications'" 
                    class="w-full flex items-center justify-between px-6 py-3.5 text-base transition-all text-left cursor-pointer border-l-4"
                    :class="activeTab === 'notifications' ? 'bg-white/10 border-blue-500 text-white font-semibold shadow-sm' : 'border-transparent text-gray-300 hover:bg-white/5 hover:text-white'">
    ...
    <div class="md:ml-64 flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header class="glass-panel h-16 flex items-center justify-between px-6 border-b">
            <h2 class="text-xl font-semibold text-slate-800" x-text="
                activeTab === 'home' ? 'User Profile' : 
                activeTab === 'forms' ? 'Forms & Templates' : 
        ...
        <main class="flex-1 overflow-y-auto p-6">
            ...
            <div x-show="activeTab === 'home'" x-transition class="space-y-6 w-full">
                ...
                <!-- Profile Card -->
                <div class="glass-panel rounded-2xl shadow-sm overflow-hidden w-full card-hover">
                    <!-- Header Banner -->
                    ...
                    <div class="px-8 pb-8 relative">
                        ...
                        <form id="profileForm" method="POST" action="{{ route('profile.update') }}" class="space-y-10" onsubmit="return confirmProfileUpdate(event)">
                            ...
                            <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-slate-200/60">
                                <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold text-base shadow-sm transition active:scale-95 cursor-pointer">Save Changes</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <!-- FORMS/DASHBOARD TAB -->
            <div x-show="activeTab === 'forms'" x-transition class="space-y-6 w-full" style="display: none;">
                <div class="glass-panel rounded-2xl shadow-sm overflow-hidden card-hover">
                    <div class="p-8 border-b border-slate-200 bg-slate-50/50">
                        <h3 class="text-xl font-bold text-slate-800">Available Templates</h3>
                        <p class="text-base text-slate-500">Download the required official IPCRF templates.</p>
                    </div>
                    
                    <div class="divide-y divide-slate-100">
                        @forelse($forms as $index => $form)
                            <div class="p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:bg-slate-50 transition-colors">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-slate-800 text-lg leading-snug">{{ $form->name }}</h4>
                                        <p class="text-slate-500 text-base mt-1">{{ $form->description ?? 'Official IPCRF document template' }}</p>
                                    </div>
                                </div>
                                
                                <a href="{{ route('forms.download', $form->id) }}" 
                                   class="bg-blue-50 text-blue-600 hover:bg-blue-100 px-5 py-3 rounded-xl font-semibold text-base flex items-center gap-2 transition-all duration-200 cursor-pointer self-start sm:self-center">
                        ...
                        @empty
                            <div class="p-12 text-center text-slate-400">
                                ...
                                <p class="text-base font-medium">No templates available at the moment.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
            ...
            <!-- NOTIFICATIONS TAB -->
            <div x-show="activeTab === 'notifications'" x-transition class="space-y-6 w-full" style="display: none;">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-xl font-bold text-slate-800">Recent Announcements</h3>
                        <p class="text-base text-slate-500 mt-1">Stay updated with official bulletins.</p>
                    </div>
                    
                    <form action="{{ route('notifications.markAllAsRead') }}" method="POST" class="m-0">
                        @csrf
                        <button type="submit" class="bg-blue-600 text-white hover:bg-blue-700 px-5 py-3 rounded-xl font-semibold text-base flex items-center gap-2 transition-all cursor-pointer shadow-sm shadow-blue-600/20 active:scale-95">
                ...
                <div class="space-y-4">
                    @forelse($dbNotices as $notice)
                        @php
                            ...
                            $createdTime = $notice->posted_at ? $notice->posted_at->diffForHumans() : '';
                        @endphp
                        <div class="glass-panel rounded-2xl shadow-sm p-8 card-hover flex items-start gap-6">
                            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 
                                @if($notice->priority === 'High') bg-red-50 text-red-600
                                @elseif($notice->priority === 'Medium') bg-yellow-50 text-yellow-600
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-4">
                                    <h4 class="font-bold text-slate-800 text-base leading-snug">{{ $notice->subject }}</h4>
                                    <span class="text-xs px-2.5 py-1 rounded-full font-semibold {{ $priorityBadge }}">
                                        {{ $notice->priority }}
                                    </span>
                                </div>
                                <p class="text-slate-600 text-sm mt-2 leading-relaxed whitespace-pre-wrap">{{ $notice->content }}</p>
                                <p class="text-slate-400 text-xs mt-3 flex items-center gap-1">
                    ...
                    @empty
                        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center text-slate-400">
                            ...
                            <p class="text-base font-medium">No announcements available yet.</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div x-show="activeTab === 'settings'" x-transition class="space-y-6 w-full" style="display: none;">
                <div class="glass-panel rounded-2xl shadow-sm p-10 card-hover">
